...
class ORFFinder:
  ...
  def longest_orf(self,direction,start_coden=['ATG'], stop_coden=['TAG','TAA','TGA']):
    if direction == "+":
      ...
      return (self.result[2], self.result[3], self.result[4], self.result[1], self.seq[self.result[2]:self.result[3]]) #CDS length, coding frame, CDS sequence
...
def little_test():
  ...
  for line in open(sys.argv[1],'r'):
    ...
    if line.startswith('>'):
  	  continue
    seq	+= line
  (a,b,l,f,s) = ORFFinder(seq).longest_orf(sys.argv[2])
  print str(a) + '\t' + str(b) + '\t' + str(l) + '\t' + str(f) + '\t' + s
