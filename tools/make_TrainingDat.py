...
parse.add_option('-m','--mRNA',dest='mRNA',action='store',metavar='mRNA files',help="enter protein-coding transcripts in .bed or .fasta format: if this is a .bed format file, '-r' must be specified; if this is a .fasta format file, ignore the '-r'.")
parse.add_option('-l','--lncRNA',dest='lncRNA',action='store',metavar='lncRNA files',help="enter long noncoding transcripts in .bed or .fasta format: if this is a .bed format file, '-r' must be specified; if this is a .fasta format file, ignore the '-r'.")
...
for file in ([options.mRNA,options.lncRNA,options.gtf,options.outfile,options.hexamer_dat]):
	if not (file):
		parse.print_help()
		sys.exit(0)

file_format = bed_or_fasta(options.mRNA)
if file_format == 'UNKNOWN':
    ...
    filearray = options.mRNA.split('.')
    inPutFileName1 = filearray[0] + '.fasta'
    TMP = open(inPutFileName1,'w')
    for line in ireader.reader(options.mRNA):
    ...
    inPutFileName1 = options.mRNA


file_format = bed_or_fasta(options.lncRNA)
if file_format == 'UNKNOWN':
	print >>sys.stderr, "\nError: unknown file format of '-g'\n"
	parse.print_help()
	sys.exit(0)		
    ...
    if not options.ref_genome:
		...
		sys.exit(0)
    index_fasta(options.ref_genome)
    filearray = options.lncRNA.split('.')
    inPutFileName2 = filearray[0] + '.fasta'
    TMP = open(inPutFileName2,'w')
    for line in ireader.reader(options.lncRNA):
		if line.startswith('track'):continue
		if line.startswith('#'):continue
		if line.startswith('browser'):continue
		#if not line.strip(): continue
    ...
    inPutFileName2 = options.lncRNA



inFiles = open(inPutFileName1)
inFilesArr1 = inFiles.read()
sequence_Arr1 = inFilesArr1.split('\n')
mRNA_num = len(sequence_Arr1) - 1
del sequence_Arr1
inFiles.close()
inFiles = open(inPutFileName2)
inFilesArr2 = inFiles.read()
sequence_Arr2 = inFilesArr2.split('\n')
lncRNA_num = len(sequence_Arr2) - 1
del sequence_Arr2
inFiles.close()
inFilesArr = inFilesArr1 + inFilesArr2
inPutFileName = 'temp_training.fasta'
outFiles = open(inPutFileName,'w')
outFiles.write(inFilesArr)
outFiles.close()
del inFilesArr1, inFilesArr2, inFilesArr 
...
if int(Parallel) == 1:
    ...
    del sequence_Arr
    mainProcess(ARRAY,outPutFileName,1,coding,noncoding,Alphabet,Matrix_hash,mRNA_num)
...
if int(Parallel) > 1:
    ...
    for i in range(1,int(Parallel)+1):
        ...
        temp_Arr = temp_inFilesArr.split('\n')
        File_num = len(temp_Arr) - 1
        del temp_Arr
        if File_num + linesnum  <= mRNA_num:
            temp_mRNA_num = File_num
        else:
            temp_mRNA_num = mRNA_num - linesnum
        linesnum = linesnum + File_num      
        
        Proc_Thread.append(Process(target=mainProcess, args=(temp_inFilesArr,outPutFileName,str(i),coding,noncoding,Alphabet,Matrix_hash,temp_mRNA_num)))
